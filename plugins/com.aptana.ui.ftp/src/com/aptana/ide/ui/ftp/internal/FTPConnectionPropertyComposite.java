...
public class FTPConnectionPropertyComposite extends Composite implements IOptionsComposite.Listener
{
			...
			Listener listener)
	{
		...
		setLayout(GridLayoutFactory.swtDefaults().margins(
				...
				converter.convertVerticalDLUsToPixels(IDialogConstants.VERTICAL_SPACING)).numColumns(2).create());
		...
		hostText.setLayoutData(GridDataFactory.swtDefaults().hint(
				new PixelConverter(hostText).convertHorizontalDLUsToPixels(IDialogConstants.ENTRY_FIELD_WIDTH),
		...
		loginCombo.setLayoutData(GridDataFactory.swtDefaults().hint(
				new PixelConverter(loginCombo).convertHorizontalDLUsToPixels(IDialogConstants.ENTRY_FIELD_WIDTH),
		...
		testButton.setLayoutData(GridDataFactory.fillDefaults().hint(
				Math.max(new PixelConverter(testButton).convertHorizontalDLUsToPixels(IDialogConstants.BUTTON_WIDTH),
		...
		remotePathText.setLayoutData(GridDataFactory.swtDefaults().hint(
				new PixelConverter(remotePathText).convertHorizontalDLUsToPixels(IDialogConstants.ENTRY_FIELD_WIDTH),
				SWT.DEFAULT).grab(true, false).create());
		...
		testButton.addSelectionListener(new SelectionAdapter()
		{
			...
			public void widgetSelected(SelectionEvent e)
			{
				if (testConnection())
				{
					MessageDialog.openInformation(getShell(), Messages.FTPConnectionPointPropertyDialog_Succeed_Title,
							MessageFormat.format(Messages.FTPConnectionPointPropertyDialog_Succeed_Message, hostText
									.getText()));
				}
			}
		});
	...
	protected void createSiteSection(Composite parent)
	{
		...
		nameText.setLayoutData(GridDataFactory.fillDefaults().hint(
				new PixelConverter(nameText).convertHorizontalDLUsToPixels(IDialogConstants.ENTRY_FIELD_WIDTH),
				SWT.DEFAULT).grab(true, false).create());
	}
	...
	protected void createPasswordSection(Composite parent)
	{
		...
		passwordText.setLayoutData(GridDataFactory.fillDefaults().hint(
				new PixelConverter(passwordText).convertHorizontalDLUsToPixels(IDialogConstants.ENTRY_FIELD_WIDTH),
				SWT.DEFAULT).grab(true, false).create());
